...
\begin{enumerate}
    ...
    \item \( \left( \mathbb{N}, \mathcal{P}(\mathbb{N}), \mu_C \right)\). \(f: \mathbb{N} \to \mathbb{R}\) is a sequence \( \left\{ a_n \right\}_{n \in \mathbb{N}}\) \[ \int_\mathbb{N} \{a_n\} \, d\mu_C = \sum_{n=0}^\infty a_n\]
\end{enumerate}
...
\end{theorem}
\begin{definition}\label{sigma additivity}
    \(\{E_n \subset \mathcal{M}\}\) disjoint, and let \(E = \bigcup_{n=1}^\infty E_n \Rightarrow s = \sum_{k=1}^m a_k \chi_{D_k} \; D_k \in \mathcal{M}
    \)
\end{definition}
\begin{proof}
    \(\mu(\emptyset) =0 \Rightarrow \phi(\emptyset)=0 \) by definition of the integral.

    Then, by definition \ref{sigma additivity} and since \(\mu\) is a measure and \(E \cap D_k = \bigcup_n (E_n \cap D_k)  \) 
    \[
        \phi(E) = \sum_{k=1}^m a_k \mu(D_k \cap E) = 
        \sum_{k=1}^m a_k \sum_{n=1}^\infty \mu(E_n \cap D_k)= 
    \]
    \[    
...
\begin{proof}
    ...
    \[
        \mu (E_n) \leq \frac{1}{\frac{1}{n}} \int_{E_n} f \, d\mu =0 \quad \forall \; n \Rightarrow \mu(E_n)=0 \quad \forall \; n 
    \]
...
\begin{remark}
    Note that, by definition of inf, \(\forall \; \epsilon > 0\) we have 
    \[
