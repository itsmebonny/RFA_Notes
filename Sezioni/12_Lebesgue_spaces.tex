...
\]
\[
    1 \leq p \leq q \leq \infty \Rightarrow \ell^p \subseteq l^q \text{, and } \exists \; C>0 \text{ s.t. } \norm{x}_q \leq C \norm{x}_p \quad \forall\; x \in \ell^p
\]
...
\end{definition}
\subsection{\texorpdfstring{\(\mathcal{S}(\real)\)}{Sr} is dense in \texorpdfstring{\(L^p\)}{Lp}}
\begin{theorem}
...
\begin{proof}
    ...
    By the simple approximation theorem, \(\exists \; \left\{ s_n \right\}\) of simple functions s.t. \(\left\{ s_n(x) \right\}\) is increasing, for every \(x\), and \(s_n \to f\) point wise in \(\real\).
