...
\begin{remark}
    ...
    \[
        \begin{array}{rlc}
            ...
            x_n \to x \in X \Rightarrow & x_n \rightharpoonup x  \in X & \\
             \nLeftarrow & 
        \end{array}
    \]
\end{remark}
\begin{remark}
    ...
    \[
        f_n \rightharpoonup f \text{ in } L^p(X) \Leftrightarrow \int_X f_n g \, d\mu \to \int_X fg \, d\mu \quad \forall \; g \in L^{{q}}
    \]
    \noindent Similarly, in \(\ell^p = L^p(\natural, \mathcal{P}(\natural), \mu_c)\)
    \[
        x_n \rightharpoonup x \text{ in } \ell^p \Leftrightarrow \sum_{k=1}^\infty x_n^{(k)} y^{(k)} \to \sum_{k=1}^\infty x^{(k)} y^{(k)} \quad \forall \; y \in \ell^q
    \]
\end{remark}
...
\end{proposition}
\begin{proof}
    By contradiction, suppose that \(\exists \{ x_n\} \subset X\) s.t. \(x_n \rightharpoonup x_1\), \(x_n \rightharpoonup x_2\)  in \(X\), \(x_1 \neq x_2\). Then
    \[
        \begin{array}{lc}
...
\begin{proposition}
    If \(x_n \rightharpoonup x\)  in \(X\), then \(\{x_n\}\) is bounded, and 
    \[
...
\begin{proof}
    \begin{itemize}
        ...
        \(x_n \rightharpoonup x\)  \(\Rightarrow \{L x_n\}\) is bounded in \(\real\), \(\forall \; L \in X^*\). Consider \(\Lambda_n \in X^{**}\) defined by 
        \[
...
\begin{proposition}
    \(x_n \rightharpoonup x \) in \(X\) , and \(L_n \to L\) (strongly) in \(X^*\). Then
    \[
...
\begin{proposition}
    ...
    \[
        x_n \rightharpoonup x  \Rightarrow Tx_n \rightharpoonup Tx 
    \]
\end{proposition}

We introduced the weak convergence. \(X\) Banach space. \({x_n} \subset X\) converges weakly to \(x\), \(x_n \rightharpoonup x\)  in \(X\), if 
\[
...
\begin{remark}
    In \(\real^n\) (or any finite dimensional Banach space) \(x_n \rightharpoonup x\)  \(\Leftrightarrow x_n \to x\) strongly (ex.)
\end{remark}
...
\[
    {}_{X^{**}}\langle\tau(x), L \rangle_{X^*} = Lx \quad \forall \; L \in X^*
\]
Thus \(L_n \rightharpoonup L\)  in \(X^*\) \(\rightarrow\) \(\langle \tau(x), L_n \rangle \to \langle \tau(x), L \rangle\) \(\forall x \in X:\) namely
\[
    L_n x \to Lx \qquad \forall \; x \in X
\]
namely \(L_n\rightharpoonup^* L\)  in \(X^*\). In general the converse is false. However
\begin{proposition}
    If \(X\) is reflexive, then \(L_n \rightharpoonup L\)  in \(X^* \Leftrightarrow L_n \rightharpoonup^* L\)  in \(X^*\)
\end{proposition}
\begin{proof}
...
\begin{proof}
    ...
    \[
        \abs{L_n x_1} \leq \norm{L_n}_{X^*} \norm{x_1}_X \leq M \norm{x_1}_X < \infty
    \]
    \(\Rightarrow \exists \; \{L_{n_j}\}\) s.t. \(L_{n_j} x_1 \to l_1\) in \(\real\). Now, consider \(\{L_{n_j} x_2\}\): it is bounded,
    \[
        \abs{L_{n_j}x_2} \leq \norm{L_{n_j}}_{X^*} \norm{x_2}_X \leq M \norm{x_2}_X < \infty
    \]
    \(\Rightarrow \exists\; \{L_{n_{ij}}\}\) subsequence of \(\{L_{n_j}\}\) s.t. \( L_{n_{ij}} x_2 \to l_2\) in \(\real\). We can iterate the process. \(\forall \; k \; \{L_n^k\}\) is a subsequence of \(\{L_n^{k-1}\}\). \(\Rightarrow \{L_n^k\}\) is a subsequence of \(\{L_n^j\}\) \(\forall j < k\). In particular, 
    \[
...
\begin{proof}
    ...
    \[
        {}_{X^{**}}\langle\tau(x_{n_k}), L \rangle_{X^*} \to {}_{X^{**}}\langle\Lambda, L \rangle_{X^*} \quad k \to \infty, \; \forall\; L \in X^*
    \]
    Since \(X\) is reflexive, \(\forall \; \Lambda \in X^{**} \) \(\exists ! \; x \in X\) s.t. \(\Lambda = \tau (x)\). Therefore, 
    \[
        L x_{n_k} = {}_{X^{**}}\langle\tau(x_{n_k}), L \rangle_{X^*} \to {}_{X^{**}}\langle \Lambda, L \rangle_{X^*} = Lx, \; \forall\; L \in X^*
    \]
    We proved that 
    \[
