...
\begin{remark}
    ...
    Take \(v(x)\), the Vitali Cantor function: \(v \in \mathcal{C}([0,1]), v(0)=0, v(1)=1\). \(v\) is differentiable a.e. on \([0,1]\), but the calculus formula doesn't hold!
\end{remark}
\begin{remark}
...
\end{example}We can now come back to the proof of Lemma 1 of the last lesson.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item \(\lambda((x,y)) = y - x \quad \forall \; a \leq x < y \leq b\)
    \end{itemize}
...
\begin{itemize}
    ...
    \item \(\mu \ll \lambda\): \(E \in \mathcal{L}([a,b])\), \(\lambda(E) = 0 \Rightarrow \mu(E) = 0\)
\end{itemize}
...
\[
    A \mbox{ is open} \Rightarrow A = \left( \bigcup_{n=1}^{\infty} I_n \symbolfootnotemark[2]{open intervals = \((x_n, y_n)\)} \right)\cap [a,b], \text{ disjoint}\] 
...
\[
    \mu (E) \leq \mu(A) = \sum_n \mu (I_n) = \sum_n \abs{G(y_n) - G(x_n)} < \epsilon
\]
...
\]
So \(\mu \ll \lambda\). We can apply Radon Nikodym \(\exists \; \oldphi : \left[ a, b\right] \to \left[0, \infty\right]\) s.t. 
\[
