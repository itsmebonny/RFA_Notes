...
\begin{definition}
    ...
    \[
        f^{-1}(E) \in \mathcal{M} \mbox{ for every } E \in \mathcal{N}
    \]
...
\begin{proposition}\label{prop_1_meas}
...
\begin{enumerate}
    ...
    \begin{example}
        \((\mathbb{R}^n, \mathcal{L}(\mathbb{R}^n))\), \((Y, d_y)\) metric space \(\to (Y, \mathcal{B}(Y))\). 
...
\begin{proposition}
    ...
    \begin{enumerate}
        ...
        \item \((Y, d_Y)\) metric space. If \(f:\mathbb{R}^n \to Y\) is continuous, then it is Lebesgue measurable.
    \end{enumerate}
\end{proposition}
\begin{proof}
    The proof is divided in:
    \begin{enumerate}
        \item This holds true: \[f \text{ is continuous }  \Leftrightarrow f^{-1}(A) \text{ is open } \forall \; A \text{ open } \subset Y \Rightarrow f^{-1}(A) \in \mathcal{B}(Y) \; \forall \; A \text{ open } \subset Y.
        \]
        Since \(\mathcal{B}(Y) = \sigma_0 \; (\mbox{open sets})\) by proposition \ref{prop_1_meas} this implies that \(f\) is Borel measurable
    ...
    \end{enumerate}
\end{proof}
\begin{proposition}\label{prop_3_meas}
...
\end{proposition}
\begin{proposition}\label{prop_4_meas}
...
\end{definition}
\begin{proof}
    Define \(f: X \to \mathbb{R}^2\), \(f(x) = (u(x), v(x))\). By definition \(h = \Phi \circ f\) by proposition \ref{prop_3_meas}. If we show that \(f\) is \((\mathcal{M},\mathcal{B}(\real^2))\)-measurable, then \(h\) is measurable. It can be proved that \[\mathcal{B}(\mathbb{R}^2) = \sigma_0 (\lbrace \underbrace{(a_1, b_1) \times (a_2, b_2)}_{\text{open rectangle}}: a,b \in \mathbb{R}\rbrace)\]
    Thanks to proposition \ref{prop_1_meas}, to check that \(f\) is measurable we can simply check that
    \(f^{-1}(R) \in \mathcal{M} \; \forall \mbox{ open rectangle in }\real^2\) and
    \(R = I \times J\), with \(I\) and \(J\) open intervals:
    \[
        \begin{array}{c}
            f^{-1}(R) = \left\lbrace x \in X : (u(x), v(x)) \in R \right\rbrace \\
            \Updownarrow \\
            ...
            \text{ since both }u,v \text{ are measurable}
        \end{array}
    \]
    This completes the proof
\end{proof}
\noindent\underline{Consequences}: by proposition \ref{prop_3_meas} and \ref{prop_4_meas}, if \(u\) and \(v\) are measurable, then also \(u+v\), \(u \cdot v\). Other measurable functions include \(u^+ = \max\left\{ u, 0 \right\}, u^- =- \min\left\{u,0\right\}, \abs{u} = u^+ + u^-, u^2, \ldots\)
...
\begin{remark}
    ...
    \]
\end{remark}
Most of the time we will work with functions \(f: X \to \real\) or \(f: X \to \underbrace{\barreal}_{\real \cup \left\{\pm \infty \right\}}\) 
...
\]
\subsection{Measurability of limits for real valued functions}
\begin{proposition}
...
\end{proposition}
\begin{proof}
    \((\sup f_n)^{-1} ((a, \infty]) = \lbrace x \in X : \underset{\mathclap{\substack{
        \Updownarrow \\
        \exists \text{ some indexes }n\text{ s.t. } f_n(x) > a
    }}}{\sup f_n(x) > a} \rbrace\) 
    \[
...
\begin{definition}
    ...
    Then 
    \[
        s(x) = \sum_{n = 1}^N a_n \chi_{E_n}(x)
    \]
    where \(E_n\) is a measurable set, \(E_n = \left\{ x \in X : s(X) = a_n \right\}\), and \(E_i \cap E_j = \emptyset\) for \(i \not = j\), and \(\bigcup_{n = 1}^N E_n = X\).
\end{definition}
\begin{example}
    If \(s: \mathbb{R} \to \barreal\), and each \(E_n\) is a finite union of intervals, then \(s\) is said to be a \textbf{step} function.
\end{example}
...
\noindent
    ...
    \]
    Then \(s_n \leq s_{n+1}\) by construction, and, since \(f\) is bounded, \(E_0 = \emptyset\) for \(n\) sufficiently large (\(n > \sup f\)).

    Then any \(x \in X\) stays in \(f^{-1}([a_j, b_j))\) for some \(j\) 
    \[
        \begin{array}{l}
            \Rightarrow \stackbelow{a_j}{s_n(x)} \leq f(x) < b_j \\
            \Rightarrow 0 \leq f(x)-s_n(x) < \underbrace{b_j - a_j}_{ = \frac{1}{2^n}} \\
...
\begin{remark}
    ...
    Note that, \(\forall \; E \in \mathcal{L}(\real) \; \exists \, \mbox{ a } G_{\delta}-\mbox{set } A\) and an \(F_{\sigma}-\mbox{set } B\) s.t.
    \[
        \begin{array}{l}
...
\end{definition}
\begin{example}
    \((\real, \mathcal{L}(\real), \lambda)\), \(f(x) = x^2\).  
